...
r"""
raw evalution script from Nice-SLAM
"""
...
def plot_traj(ax, stamps, traj, style, color, label, 
              add_ticks=False, markers_on=None):
    """
    ...
    for i in range(len(stamps)):
        ...
        last = stamps[i]
    if len(x) > 0:
        if markers_on is not None:
            ax.plot(x, y, style, color=color, label=label, markevery=markers_on, marker="o")
        elif add_ticks:
            markers_on = list(range(0,len(x),25))
            ax.plot(x, y, style, color=color, label=label, markevery=markers_on, marker="o")
            for m in markers_on:
                ax.annotate(f"{m}", (x[m],y[m]), textcoords="offset points", xytext=(0,10),ha="center")
        else:
            ax.plot(x, y, style, color=color, label=label)
...
def evaluate_ate(first_list, second_list, plot="", _args=""):
    ...
    if args.plot:
        ...
        ATE *=100.0
        ATE = numpy.around(ATE, decimals=2)
        ax.set_title(f'num_points: {len(trans_error)} || ATE-RMSE: {ATE} cm')
        plot_traj(ax, 
                  first_stamps, 
                  first_xyz_full.transpose().A,
                  '-', "black", "ground truth", add_ticks=True)
        plot_traj(ax, 
                  second_stamps, 
                  second_xyz_full_aligned.transpose().A, 
                  '-', "blue", "estimated-aligned")
        
        plot_traj(ax, 
                  second_stamps, 
                  second_xyz_full.transpose().A, 
                  '-', "red", "estimated-raw")
